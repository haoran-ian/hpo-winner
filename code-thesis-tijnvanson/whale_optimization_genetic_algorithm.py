...
class WhaleOptimizationGeneticAlgorithm():
    ...
    def optimize(self):
        ranked_sol = self._rank_solutions() #first element is best solution
        self._local_best = ranked_sol[0]  #we set local best for first iteration
        self._global_best = ranked_sol[0] #we set global best for first iteration
        
        while self.budget > 0:
            nr_elites = int(len(ranked_sol) * self._er) #calculate the number of solutions getting a spot for the next generation
            elites = ranked_sol[:nr_elites] #determing the solutions
            ...
            binary_sols = self._encode(new_sols) #we encode the solutions to their binary representations
            
            binary_sols = self._wheel_of_fortune(binary_sols, new_sols) #we peform selection
            binary_sols = self._one_point_crossover(binary_sols) #we perform single-point crossover
            binary_sols = self._bit_mutation(binary_sols) #we perform mutation
            
            decoded_sols = self._decode(binary_sols) #the solutions get decoded to their respective form
            self._sols = np.array(elites + decoded_sols) #the elites which were reserved + the newly obtained solutions
            ranked_sol = self._rank_solutions() 
            self._local_best = ranked_sol[0] #update local best
            
            if self.problem(self._local_best) < self.problem(self._global_best): #compare if global best needs to be updated
    ...
    def _wheel_of_fortune(self, binary_sols, new_sols):
        ...
        inverted_fitness = 1.0 / (fitnesses + epsilon) #because we need the lowest fitness to have the highest probability 
                                                        #we take the inverse of the fitness

        probabilities = inverted_fitness / np.sum(inverted_fitness) #calculating probabilities
    ...
    def _crossover(self, a, b, index): #this performs the actual crossover with the determined index
    ...
    def _one_point_crossover(self, binary_sols):
        ...
        for i in range(0, len(binary_sols), 2):
            if i + 1 < len(binary_sols):
                parent1 = binary_sols[i] #the adjecent elements are chosen as the parents
                parent2 = binary_sols[i + 1]
                if np.random.rand() < self._pc: #each solution has a chance of getting chosen for crossover based in pc
                    index = random.randint(1, len(parent1) -1) #index is randomly chose
                    child1, child2 = self._crossover(parent1, parent2, index)
                    sols.extend([child1, child2])
                else:
                    sols.extend([parent1, parent2]) #if no crossover just extend the parents
            else:
                sols.append(binary_sols[i])
        return sols
    
    def _mutate(self, bit):
        if np.random.rand() < self._pm: #each bit has a chance of getting mutated
            return '1' if bit == '0' else '0'
        return bit 
    
    def _bit_mutation(self, binary_sols): #this function goes over all bits
        sols = []
    ...
    #this function encodes from floating point to binary form                
    def _encode(self, new_sols):
        ...
        for sol in new_sols:
            ...
            for c, s in zip(self._constraints, sol):
                c_min = c[0]
                c_max = c[1]
                normalized = (s - c_min) / (c_max - c_min) #we nromalize the solution based on constraints
                scaled = int(normalized * (2 ** 32 - 1)) 
                binary_s += format(scaled, '032b') #we need to set a format for 32 bits. 
            binary_sols.append(binary_s)
        return binary_sols

    #this function decodes from binary strings to floating point 
    def _decode(self, binary_sols):
        ...
        for binary_s in binary_sols:
            ...
            for i in range(len(self._constraints)):
                ...
                denormalized = decimal / (2 ** 32 - 1) * (c_max - c_min) + c_min #we denormalize
                sol.append(denormalized)
            sols.append(sol)
        return sols 

    #rest of the function is the same as in original WOA
